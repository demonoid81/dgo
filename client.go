...
import (
	...
	"crypto/x509"
	"fmt"
	"github.com/demonoid81/dgo/protos/api"
	"math/rand"
	"net/url"
	"strings"
	"sync"
...
func NewGraphClient(clients ...api.GraphClient) *Graph {
	dg := &Graph{
		dc: clients,
	}
...
func DialSlashGraphQLEndpoint(endpoint, key string) (*Graph, error) {
	...
	dc := api.NewGraphClient(conn)
	dg := NewGraphClient(dc)
...
func (d *Graph) login(ctx context.Context, userid string, password string,
...
func (d *Graph) Login(ctx context.Context, userid string, password string) error {
	return d.login(ctx, userid, password, 0)
}

// LoginIntoNamespace logs in the current client using the provided credentials.
// Valid for the duration the client is alive.
func (d *Graph) LoginIntoNamespace(ctx context.Context, userid string, password string,
	namespace uint64) error {
...
//   2. Drop a predicate.
//   3. Drop the database.
func (d *Graph) Alter(ctx context.Context, op *api.Operation) error {
	dc := d.anyClient()
...
func (d *Graph) Relogin(ctx context.Context) error {
	return d.retryLogin(ctx)
}

func (d *Graph) retryLogin(ctx context.Context) error {
	d.jwtMutex.Lock()
	defer d.jwtMutex.Unlock()
...
func (d *Graph) getContext(ctx context.Context) context.Context {
	d.jwtMutex.RLock()
	defer d.jwtMutex.RUnlock()
...
func (d *Graph) anyClient() api.GraphClient {
